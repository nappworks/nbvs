import std/random
import nbvs/[wavelet_matrix, wavelet_matching_runs]
import ./test_common
...
block simpleRuns:
  ...
  doAssert wm.collectMatchingRuns(7) == @[
    (left: 1'i64, right: 4'i64),
    (left: 5'i64, right: 7'i64)]
  doAssert wm.collectMatchingRuns(1) == @[
    (left: 0'i64, right: 1'i64),
    (left: 4'i64, right: 5'i64)]
  doAssert wm.collectMatchingRuns(7, 2, 6) == @[
    (left: 2'i64, right: 4'i64),
    (left: 5'i64, right: 6'i64)]

block edgeCases:
  let empty = genWaveletMatrix(newSeq[uint64]())
  doAssert empty.collectMatchingRuns(0).len == 0

  let zeros = genWaveletMatrix(@[0'u64, 0, 0, 0])
  doAssert zeros.collectMatchingRuns(0) == @[
    (left: 0'i64, right: 4'i64)]
  doAssert zeros.collectMatchingRuns(1).len == 0

  let wm = genWaveletMatrix(@[1'u64, 2, 3])
  doAssert wm.collectMatchingRuns(100).len == 0
  doAssert wm.collectMatchingRuns(2, 1, 1).len == 0
  expectRaises(IndexDefect): discard wm.collectMatchingRuns(2, -1, 2)
  expectRaises(IndexDefect): discard wm.collectMatchingRuns(2, 0, 4)
  expectRaises(IndexDefect): discard wm.collectMatchingRuns(2, 2, 1)

block prefixSplits:
  let xs = @[7'u64, 6, 7, 7, 4, 7, 6, 7]
  let wm = genWaveletMatrix(xs, 3)
  doAssert wm.collectMatchingRuns(7) == naiveRuns(xs, 7, 0, xs.len)
...
block randomRuns:
  ...
  for trial in 0..<120:
    ...
    for sample in 0..<40:
      ...
      doAssert wm.collectMatchingRuns(value, int64(left), int64(right)) ==
        naiveRuns(xs, value, left, right)

    for value in 0'u64..20'u64:
      doAssert wm.collectMatchingRuns(value) == naiveRuns(xs, value, 0, xs.len)
