...
proc consumeMatchingRuns(wm: WaveletMatrix, target: uint64): int64 =
...
proc measure(wm: WaveletMatrix, target: uint64, repeats: int): Sample =
  ...
  var matchingRunsSamples, regularSelectSamples, cursorSelectSamples: seq[int64]
  var sink = 0'i64

  for _ in 0..<repeats:
    var started = getMonoTime()
    sink = sink xor consumeMatchingRuns(wm, target)
    matchingRunsSamples.add (getMonoTime() - started).inNanoseconds
  ...
  result.matchingRunsNs = percentile(matchingRunsSamples, 0.50)
...
proc main() =
  ...
  echo "rows,run_length,occurrences,runs,repeats,matching_runs_p50_ns," &
    "regular_select_p50_ns,cursor_select_p50_ns," &
    "matching_vs_regular_speedup,matching_vs_cursor_speedup"
  ...
  for runLength in [1, 4, 16, 64, 256, 1024, 4096]:
    ...
    let occurrences = wm.rank(target, 0, wm.n)
    var runCount = 0'i64
    for _ in wm.matchingRuns(target):
      inc runCount

    let sample = measure(wm, target, repeats)
    let regularSpeedup = if sample.matchingRunsNs == 0: 0.0 else:
      float(sample.regularSelectNs) / float(sample.matchingRunsNs)
    let cursorSpeedup = if sample.matchingRunsNs == 0: 0.0 else:
      float(sample.cursorSelectNs) / float(sample.matchingRunsNs)

    echo &"{rows},{runLength},{occurrences},{runCount},{repeats}," &
      &"{sample.matchingRunsNs},{sample.regularSelectNs},{sample.cursorSelectNs}," &
      &"{regularSpeedup:.3f},{cursorSpeedup:.3f}"
